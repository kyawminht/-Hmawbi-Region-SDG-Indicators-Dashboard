import React, { useState, useRef } from 'react';
 ...
 const RegionMap = ({ regions, selectedRegion, onRegionClick }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-3 md:p-4 h-full flex flex-col overflow-hidden">
      {/* Header - Fixed */}
      ...
      <div className="flex-1 min-h-0 w-full relative" ref={svgContainerRef}>
        {/* Zoom Controls */}
        <div className="absolute top-2 right-2 z-10 flex flex-col gap-1 bg-white rounded-lg shadow-md">
          <button
          ...
          <button
            onClick={handleReset}
            className="w-8 h-8 flex items-center justify-center hover:bg-gray-100 rounded-b-lg border-t text-xs font-medium"
            title="Reset View"
          >
            ↺
          </button>
        ...
        <div className="absolute bottom-2 right-2 z-10 bg-white/90 backdrop-blur-sm px-2 py-1 rounded text-xs shadow-md">
        ...
        >
          ...
          >
            ...
            >
            {/* All your polygon groups remain exactly the same */}
            {/* မြောင်းတကာ RHC */}
     ...
     {/* Compact Facility Information - Optimized for both mobile and desktop */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-4 p-3 bg-gray-50 rounded-lg flex-shrink-0">
      ...
      {selectedRegion && (
        <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded-lg md:hidden flex-shrink-0">
